...
[Think live message cache — `messages` getter, cache helpers, and `AbortRegistry`](../packages/think/src/think.ts#L800-L1000) — Think's `messages` property returns `_cachedMessages`, a live in-memory mirror of the durable session storage. `_syncMessages()` reloads the cache from storage at safe boundaries. `_upsertCachedMessage()` and `_patchCachedMessage()` keep the cache coherent after individual message writes. `appendMessageToHistory()`, `updateMessageInHistory()`, and `upsertMessageInHistory()` are the protected API for writing individual messages while keeping the cache in sync. Also contains the `AbortRegistry` instantiation and private config storage helpers (`think_config` table) used by Think's own settings.

[`_runInferenceLoop()` — assembling context and calling `streamText()`](../packages/think/src/think.ts#L1200-L1499) — the core inference method. Merges workspace tools, `getTools()`, extension tools, session context tools, MCP tools, and client tools into a single `ToolSet`. Freezes the system prompt via `session.freezeSystemPrompt()` and appends Think's capability block. Calls `beforeTurn()` and pipes the result through the extension `beforeTurn` pipeline. Wraps every tool's `execute` with `_wrapToolsWithDecision()` so `beforeToolCall` can intercept. Calls `streamText()` with `prepareStep`, `onChunk`, `onStepFinish`, and `experimental_onToolCallFinish` wired to Think's lifecycle hooks and extension pipeline.

[`_runInferenceLoop()` continued — extension pipeline dispatchers and `_wrapToolsWithDecision()`](../packages/think/src/think.ts#L1500-L1799) — `_pipelineExtensionBeforeTurn()` fans out the `beforeTurn` override to subscribed extension Workers with last-write-wins merging. `_dispatchExtensionObservation()` handles fire-and-forget observation hooks (`beforeToolCall`, `afterToolCall`, `onStepFinish`, `onChunk`) with per-extension timeouts. `_wrapToolsWithDecision()` wraps each tool's `execute` so `beforeToolCall` runs first: `allow` passes through, `block` returns `reason`, `substitute` returns the provided `output` — all with correct handling for async-iterable streaming tools.

[Extension pipeline dispatchers — `_pipelineExtensionToolCallStart/Finish/StepFinish/Chunk`](../packages/think/src/think.ts#L1800-L2000) — thin helpers that check extension subscriptions, lazily import the `hook-proxy` snapshot factories, and call `_dispatchExtensionObservation` with the serialisable snapshot. These are the glue between Think's lifecycle events and sandboxed extension Workers.

[Extension observation dispatch and host bridge methods](../packages/think/src/think.ts#L2000-L2100) — remaining extension pipeline helpers (`_pipelineExtensionToolCallFinish`, `_pipelineExtensionStepFinish`, `_pipelineExtensionChunk`). Followed by the host bridge methods that extension Workers invoke via `HostBridgeLoopback` RPC: `_hostReadFile`, `_hostWriteFile`, `_hostDeleteFile`, `_hostListFiles`, `_hostGetContext`, `_hostSetContext`, `_hostGetMessages`, `_hostSendMessage`, `_hostGetSessionInfo`.

[Sub-agent RPC entry point — `chat()` method, `getMessages()`, `clearMessages()`](../packages/think/src/think.ts#L2100-L2200) — `chat()` is the entry point when Think is called as a sub-agent over RPC. It accepts a user message (string or `UIMessage`), appends it to history, runs the full inference loop via `_runInferenceLoop()`, and streams `UIMessageChunk` events to the caller via a `StreamCallback`. `getMessages()` and `clearMessages()` are public message-access methods.

[Think recovery — stream finalization, fiber recovery, and `_chatRecoveryContinue`](../packages/think/src/think.ts#L4400-L4699) — `_streamResult()` streams chunks from the inference loop to connected WebSocket clients via `StreamAccumulator` and `ResumableStream`, broadcasts the final `done` signal, and persists the completed assistant message. Also contains `_streamResultToRpcCallback()` for sub-agent streaming. At L4730, `_handleInternalFiberRecovery()` intercepts interrupted `__cf_internal_chat_turn` fibers after hibernation, calls `onChatRecovery()`, persists any orphaned stream, and schedules a recovery continuation.

[Think recovery — `_chatRecoveryContinue`, `_chatRecoveryRetry`, `_getPartialStreamText`, and submission recovery helpers](../packages/think/src/think.ts#L4700-L4999) — `_chatRecoveryContinue()` resumes an interrupted turn by calling `continueLastTurn()`. `_chatRecoveryRetry()` re-runs the last unanswered user turn. `_getPartialStreamText()` replays stored chunks through `applyChunkToParts` to reconstruct partial message content. Also contains submission recovery helpers used when a `cf_think_submissions` row was interrupted mid-execution.

[Submission management internals — `_runningSubmissionByRequestId`, `_markRecoveredSubmissionInterrupted`, `_completeRecoveredSubmission`, `onChatRecovery`, and `_chatRecoveryContinue`](../packages/think/src/think.ts#L5000-L5299) — SQLite helpers for finding and updating `cf_think_submissions` rows during recovery. `onChatRecovery()` (Think's override) integrates submission lifecycle with fiber recovery: marks interrupted submissions as `error` or hands them to `_completeRecoveredSubmission` when the stream was already flushed.

[Auto-continuation, response hook, resume helpers, and `_persistOrphanedStream`](../packages/think/src/think.ts#L5300-L5421) — `_scheduleAutoContinuation()` and `_fireAutoContinuation()` implement Think's tool auto-continuation with a 50 ms coalesce timer. `_fireResponseHook()` calls `onChatResponse()` once per turn. `_notifyStreamResuming()` sends `CF_AGENT_STREAM_RESUMING`. `_persistOrphanedStream()` reconstructs and persists a partial assistant message from chunks left in a ResumableStream after hibernation.
