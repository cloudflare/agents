...
[`addMcpServer()` HTTP/SSE transport path, `removeMcpServer()`, and `getMcpServers()`](../packages/agents/src/index.ts#L9330-L9629) — the HTTP transport branch of `addMcpServer`: resolves `callbackHost`/`callbackPath` from options or the current request context, constructs the OAuth callback URL, creates the `DurableObjectOAuthClientProvider`, registers the server with `mcp.registerServer`, connects, and triggers capability discovery. Returns either an auth URL (OAuth required) or `READY`. Also `removeMcpServer`, and `getMcpServers()` (assembles the `MCPServersState` snapshot from the `MCPClientManager`).

[`createMcpOAuthProvider`, OAuth callback handling, `routeAgentEmail`, and `routeAgentRequest`](../packages/agents/src/index.ts#L9630-L9836) — `createMcpOAuthProvider` (override point for custom OAuth providers, default uses `DurableObjectOAuthClientProvider`). `broadcastMcpServers` (sends the MCP state frame to all protocol-enabled connections). `handleMcpOAuthCallback` (checks whether an incoming HTTP request is an OAuth callback, exchanges the code, re-establishes the connection, broadcasts updated state). `handleOAuthCallbackResponse` (builds the redirect or custom response for the OAuth completion). Closes the `Agent` class. Then outside the class: `EmailBridge` (`RpcTarget` that wraps `ForwardableEmailMessage` mutation methods into a disposable RPC target), `routeAgentEmail` (resolves the agent namespace and name from the email resolver, then calls `agent._onEmail()` with the bridge), and `routeAgentRequest` (thin wrapper around `routePartykitRequest` with `prefix: "agents"`).
