...
The root `.oxlintrc.json` registers the plugin. The generic rules that fit the
current codebase run as errors. Rules that require a broader
...
| `no-object-parameters` | 14 | Enabled after tightening simple cases, with file-level exceptions for private instrumentation bridges, dynamic provider objects, and prototype-scanning host helpers. |
| `no-reflect-apply` | 8 | Enabled, with exceptions for proxy forwarding and prototype-call tests and the WebSockets callables dispatcher, which must preserve the receiver. |
| `no-reflect-get` | 4 | Enabled, with exceptions for proxy traps and callables dispatch that must preserve receiver semantics. |
...
The upstream Effect-specific plugin is not vendored. This repository does not
use Effect in its packages, so its rules would have nothing to check.
