// Vendored from packages/agents/src/mcp/x402.ts @ 762998da, ISSUE-003.
/**
...
// Mechanical edit (ISSUE-003): type-only import kept on the ROOT "zod" (v3
// here), NOT "zod-v4" — the MCP SDK's ToolCallback/registerTool typings bind
// to whatever zod the root resolves, so ZodRawShape must come from the same
// resolution or the Args constraint fails. (In the original monorepo the root
// zod WAS v4, so this line originally read `from "zod"` too.)
...
export function withX402<T extends McpServer>(
  ...
  function ensureInitialized(): Promise<void> {
    if (!initPromise) {
      initPromise = resourceServer.initialize().catch((err: unknown) => {
