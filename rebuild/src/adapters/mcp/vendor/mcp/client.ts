// Vendored from packages/agents/src/mcp/client.ts @ 762998da, ISSUE-003.
...
export class MCPClientManager {
  ...
  async connect(
    ...
    if (!connection) {
      throw new Error(`MCP connection ${id} was not created`);
    }
    ...
    if (options.reconnect?.oauthCode) {
      try {
        ...
        let completeError: unknown;
        try {
          await connection.completeAuthorization(options.reconnect.oauthCode);
        } catch (error) {
          completeError = error;
        }
  ...
  async handleCallbackRequest(req: Request): Promise<MCPOAuthCallbackResult> {
    ...
    if (!conn) {
      throw new Error(`Missing MCP connection for OAuth callback: ${serverId}`);
    }
  ...
  readResource(
    ...
    return connection.client.readResource(params, options);
  }
  ...
  getPrompt(
    ...
    return connection.client.getPrompt(params, options);
  }
}
