 ...
 * - last original change: b58b5a3cb5eba150e12bff2ef6dbfebd1bed6248
 * - port date: 2026-07-15
 * Modifications:
 * - Re-pointed the original `agents` import to `./compat.js`.
 * - Renamed fixture bindings/classes to hooks-specific names to avoid
 *   collisions with already-ported fixture waves; test bodies/assertions stay
 *   otherwise verbatim.
 * - Added `@ts-nocheck` because the original test predates this rebuild's
 *   stricter RPC/env typing.
 */
// @ts-nocheck
...
async function freshAgent(name: string) {
  return getAgentByName(env.ThinkHooksAgent, name);
}

async function freshProgrammaticAgent(name: string) {
  return getAgentByName(env.ThinkHooksProgrammaticAgent, name);
}

async function freshToolAgent(name: string) {
  return getAgentByName(env.ThinkHooksToolsAgent, name);
}

async function freshLoopToolAgent(name: string) {
  return getAgentByName(env.ThinkHooksLoopToolAgent, name);
}
...
describe("Think — actions compile into guarded tools", () => {
  ...
  it("emits an approval request with a stable action descriptor", async () => {
    ...
    const ws = await connectWS("ThinkHooksToolsAgent", room);
...
async function freshExtensionHookAgent(name: string) {
  return getAgentByName(env.ThinkHooksExtensionAgent, name);
}
...
async function freshSessionAgent(name: string) {
  return getAgentByName(env.ThinkHooksSessionAgent, name);
}
...
describe("Think — beforeTurn config overrides", () => {
  ...
  it("applies sendReasoning on the WebSocket stream path", async () => {
    ...
    const ws = await connectWS("ThinkHooksAgent", room);
