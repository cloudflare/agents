 ...
 * - packages/think/src/tests/actions-durable-pause.test.ts
 * - last original change: unknown
 * - port date: 2026-07-15
 * Modifications:
 * - Re-pointed `agents` and `../think` imports to `./compat.js`.
 * - Added `@ts-nocheck` because the original test predates this rebuild's
 *   stricter Workers RPC typing.
 */
// @ts-nocheck
import { describe, expect, it, vi } from "vitest";
import { env } from "cloudflare:workers";
import { getAgentByName, action } from "./compat.js";
