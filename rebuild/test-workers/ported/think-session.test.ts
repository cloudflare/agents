// @ts-nocheck
// Ported from ../packages/think/src/tests/think-session.test.ts
// Original commit: unknown in clean-room port (source file read-only input for audit 29 T3).
// Port date: 2026-07-15
// Modifications: compat imports, rebuilt fixture module, 10s timeout, native-coverage skips, clean-room public API rewrites via fixtures.
import { env, exports } from "cloudflare:workers";
import { getServerByName, subscribe, type ChatResponseResult, type SaveMessagesResult } from "./compat.js";
import { describe, expect, it, vi } from "vitest";
vi.setConfig({ testTimeout: 10000 });
import type { UIMessage } from "ai";
import type {
  ThinkSessionThinkTestAgent as ThinkTestAgent,
  ...
  ThinkSessionRecoveryAgent as ThinkRecoveryTestAgent,
  ThinkNonRecoveryTestAgent,
  TestChatResult
} from "./fixtures/think-session-agents.js";
...
async function connectThinkTestAgentWS(room: string): Promise<WebSocket> {
  const response = await exports.default.fetch(
    `http://example.com/agents/think-session-think-test-agent/${room}`,
...
async function freshRecoveryAgent(name: string) {
  return getServerByName(
    env.ThinkSessionRecoveryAgent as unknown as DurableObjectNamespace<ThinkRecoveryTestAgent>,
    name
  );
}
...
describe("Think — core", () => {
  it.skip("should run a chat turn and persist messages", async () => {
    // dropped: native coverage in src/app/think.test.ts — text turn persists final transcript and history.
  ...
  it.skip("should clear all messages", async () => {
    // dropped: native coverage in src/app/think.test.ts — clearMessages publishes and empties history.
  ...
  it.skip("should stream events via callback", async () => {
    // dropped: native coverage in src/app/think.test.ts — chat callback receives onStart/onEvent/onDone.
...
describe("Think — dynamic configuration", () => {
  it.skip("should persist and retrieve typed configuration", async () => {
    // dropped: native coverage in src/app/think.test.ts — configure()/getConfig() persist server-private config.
  ...
  it.skip("should overwrite configuration on re-configure", async () => {
    // dropped: native coverage in src/app/think.test.ts — configure()/getConfig() overwrite semantics are covered.
...
describe("Think — chatRecovery", () => {
  it.skip("chat turn with recovery=true works normally and cleans up fibers", async () => {
    // dropped: native coverage in src/app/think.test.ts — recovery-enabled turns complete and clean up.
  ...
  it.skip("recovery=false works without creating fiber rows", async () => {
    // dropped: native coverage in src/app/think.test.ts — chatRecovery false skips fiber wrapping and completes.
