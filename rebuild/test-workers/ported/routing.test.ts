 ...
 * - packages/agents/src/tests/routing.test.ts
 * - last original change: 36e2020d
 * - port date: 2026-07-15
 * Modifications:
 * - Kept worker-level fetch assertions; fixture worker re-creates only the
 *   original bindings needed by this file.
 */
...
describe("routeAgentRequest", () => {
  describe("URL pattern matching", () => {
    ...
    it("should return 404 for malformed paths", async () => {
      ...
      // Missing agent name
      const res2 = await worker.fetch("http://example.com/agents/");
      expect(res2.status).toBe(404);

      // Just /agents
      const res3 = await worker.fetch("http://example.com/agents");
