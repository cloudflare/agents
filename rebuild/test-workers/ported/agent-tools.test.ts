 ...
 * - last original change: f5998923
 * - port date: 2026-07-15
 * Modifications:
 * - Re-pointed `agents` imports to `./compat.js` and fixture type imports to
 *   `./fixtures/index.js`.
 * - Defined the two progress/milestone constants locally because the rebuild
 *   compat shim does not expose the old `agents` package constants.
 * - The fixture adapts original caller-supplied run ids to the rebuild's
 *   public API, which always mints run ids internally.
 */
// @ts-nocheck
import { env } from "cloudflare:workers";
import { getAgentByName } from "./compat.js";
import { describe, expect, it } from "vitest";
import type { ThinkAgentToolParent, ThinkTestAgent } from "./fixtures/index.js";

const AGENT_TOOL_PROGRESS_PART = "agent-tool-progress";
const AGENT_TOOL_MILESTONE_PART = "agent-tool-milestone";
...
type AgentToolLifecycleResult = Record<string, unknown>;
type AgentToolRunInfo = Record<string, unknown>;
type RunAgentToolResult = Record<string, unknown>;
