 ...
 * - packages/think/src/tests/client-tools.test.ts
 * - last original change: 190ea814
 * - port date: 2026-07-14
 * Modifications:
 * - Re-pointed `agents`, `agents/observability`, and `../think` imports to `./compat.js`.
 * - Added `@ts-nocheck` because the original test predates this rebuild's stricter indexed-access typing.
 */
// @ts-nocheck
import { env, exports } from "cloudflare:workers";
import { describe, expect, it } from "vitest";
import type { UIMessage } from "ai";
import { getAgentByName, subscribe, type ChatResponseResult } from "./compat.js";
...
async function connectWS(room: string) {
  const res = await (exports as { default: { fetch: typeof fetch } }).default.fetch(
