 ...
 * - packages/agents/src/tests/message-handling.test.ts
 * - port date: 2026-07-15 (P12) [fidelity:adapter]
 * Modifications:
 * - Re-pointed `MessageType` import to `./compat.js`.
 * - Reuses the P12 callable fixture (`TestCallableAgent`, binding
 *   `test-callable-agent`, with `add`/`voidMethod`) and the shared
 *   `TestNoIdentityAgent` fixture (identity opt-out).
 * - Assertions kept verbatim, including original frame-vocabulary
 *   expectations (identity `agent` field, `cf_agent_mcp_servers` on
 *   connect) — these fail honestly where the rebuild's connect sync
 *   differs (identity carries `className`, no MCP servers frame).
 */
// @ts-nocheck
...
describe("message handling edge cases (ported)", () => {
