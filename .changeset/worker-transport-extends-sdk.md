...
- **Stateless transports can no longer be reused across requests.** When constructed without a `sessionIdGenerator`, the SDK now throws `"Stateless transport cannot be reused across requests"` on the second `handleRequest`, because reuse causes JSON-RPC id collisions between clients. The old hand-rolled transport silently allowed reuse. Create a new transport per request (which `createMcpHandler` already does).
- **`storage` alone no longer makes a transport stateful.** Supplying `storage` persists state but does not, by itself, require an `mcp-session-id` header on later requests — statefulness is still driven solely by `sessionIdGenerator`, matching the original intent.
