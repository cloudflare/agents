...
Tool approval is decided asynchronously — `needsApproval` may return a promise and v7's `toolApproval` policy always does — so its spans can be opened after the invocation that asked for the decision has ended. They are now marked like any other span in that position, instead of being emitted unflagged under a parent that has already closed. A span that opens and closes within a single tick previously ignored the lifetime policy altogether, which is why zero-duration approval segments slipped through.
