...
- `runtime.fork()` snapshots an execution into an independent branch (checkpoint / hand-off)
...
**Runtime-facing tool** — `createCodemodeRuntime({ ctx, executor, connectors }).tool()` returns one `{ code }` tool. Inside the sandbox: `codemode.search/describe/connectors/pending/run/save/snippets/step/fork/get/set` plus `<connector>.<method>(...)` globals.
