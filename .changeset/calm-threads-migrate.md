...
Attachment offload is on by default; `sessionAttachments` is now a policy override rather than the switch that enables it. Payloads are extracted out of a message row only when an R2 bucket is configured and the payload is at or above `r2ThresholdBytes`, or when the row would otherwise exceed 1.5 MiB.
