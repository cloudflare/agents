...
- `ChatRecoveryExhaustedContext` (the `onExhausted` argument) now carries
  ...
  and `partialText` / `partialParts` (what the turn produced before it was given
  up on) — enough to render or persist a user-facing terminal banner directly.

Both fields are additive. Applied across `agents` (shared types),
