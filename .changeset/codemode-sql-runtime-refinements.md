...
- **Lifecycle hardening.** `deleteExecution` on a non-terminal run also disposes its per-execution connector resources. Oversized execution code is rejected up front with a model-actionable error, and snippet schemas are size-checked on save. `createCodemodeRuntime` rejects duplicate connector names (they would silently shadow each other in the sandbox).
