...
`mediaEviction: false` now means the model keeps seeing aged media. It no longer changes where Sessions keeps the bytes, and it leaves Sessions' own aged-row maintenance at its default. With eviction on, Sessions' maintenance pass is switched off, because Think owns aged-row policy.

`minPartBytes` is now purely Think's context threshold and is no longer passed to Sessions as a storage setting. Where Sessions keeps a payload is decided by `sessionAttachments.r2ThresholdBytes` and the 1.5 MiB row budget alone.
