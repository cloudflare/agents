...
Connectors define the service and execute tools. The generic `CodemodeSession` facet delegates execution back to the connector — so the connector owns both definition and execution, while the session owns state, approvals, and simulation.
