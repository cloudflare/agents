...
| `mediaHydration`           | `{ keepRecentMessages: 4 }`      | How many of the newest messages hold their media bytes in memory; older ones keep Sessions pointers and reach the model as a marker. `"all"` hydrates every message in the window                                                                                                                                                                                                                                                                                                                                           |
...
Within that window, only the newest `mediaHydration.keepRecentMessages` messages (4 by default, clamped to the four the model replays at full fidelity) are held with their media bytes. Older messages keep their Sessions pointers (`attachment:sha256:…`) in the live cache: the model was sent those images when they were recent, and a copy the isolate holds for its lifetime, serializes to every client on every broadcast, and walks on every turn is pure cost. Before a turn, a part still carrying a pointer reaches the model as a marker:

```
[image/png omitted: older than the recent window]
```

Nothing durable changes — the row keeps its pointer and `session.getHistory()` still returns the bytes — and a pointer-form message can be written back (a tool update, a transcript repair) without losing its attachment. The visible cost is on the wire: a client receives a message older than the window with its pointer rather than its bytes, so it cannot render that image from the transcript alone. `mediaHydration: "all"` restores full hydration for every message in the window.
