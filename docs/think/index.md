...
| `skillsRefresh`            | `{ intervalMs: 60_000 }`         | When skill sources are re-listed: `{ intervalMs }`, `"every-turn"`, or `"on-start"` (load once). Continuations of an in-flight turn never refresh; a catalog change re-renders the system prompt for the next turn only                                                                                                                                                                                                                                                                                                     |
