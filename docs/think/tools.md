...
4. **Context tools** — `set_context`, `load_context`, `search_context` (from `configureContext`)
...
Extensions can declare context blocks in their manifest. These are automatically added to the agent's context blocks:
