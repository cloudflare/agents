...
Calls in both directions can carry the caller: pass `{ rpc: "contextual" }`
to `dynamicAgents.get()`, `subAgent()`, or `parentAgent()` and the called
method's `getCurrentAgent().caller` names the calling agent. See
