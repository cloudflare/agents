...
| `ctx.ask(kind, payloads, { expiresIn? })`     | Raise durable questions, one row each, synchronously; returns `Pending<A>[]` — `{ id }` objects that belong in the checkpoint of the phase that reads them. Ids derive from the turn and the kind, so a replayed transition finds its own asks. |
...
| `ctx.spawn(definition, input, options)`       | Accept a child run of this run. `background: true` detaches it: outside the cancel cascade, silent unless `notify: true`. Without `runId` the child's id derives from the turn, so a replay joins it.                                           |
