...
Most examples have both a frontend and a backend. This makes them immediately runnable and visually demonstrable — users can `vp run start` and see the feature in action, not just read server logs.
...
Generated by `vp exec wrangler types`. Do not hand-edit. Regenerate when bindings change.
...
2. How to run it (`vp install && vp run start` for full-stack examples, `vp install && vp run dev` for server-only examples)
