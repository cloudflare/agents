...
import { implementedScenarioIds } from "../e2e/manual/coverage";
...
// All scenarios from testing.md are covered by manual specs — nothing to generate.
test.describe("generated stubs", () => {});
`;
  }
...
const uncoveredScenarios = scenarios.filter(
  (scenario) => !implementedSet.has(scenario.id)
);

writeFileSync(manifestPath, `${jsonString(scenarios)}\n`);
writeFileSync(generatedSpecPath, generateSpec(uncoveredScenarios));

console.log(
  `Parsed ${scenarios.length} scenarios from ${testingPath.replace(`${playgroundDir}/`, "")}; ${uncoveredScenarios.length} uncovered stubs generated`
);
