...
curl http://localhost:8787/objects/mcp-client-object/demo

# Connect to a public MCP server.
curl -X POST \
  http://localhost:8787/objects/mcp-client-object/demo/connect \
  -H 'content-type: application/json' \
  -d '{"name":"docs","url":"https://docs.mcp.cloudflare.com/mcp"}'

# Remove the persisted server.
curl -X DELETE \
  http://localhost:8787/objects/mcp-client-object/demo/servers/docs
```
...
The `/objects/mcp-client-object/demo/callback` shape comes from the default
`routeDurableObjectRequest()` prefix, binding name, and object name. It is not
required by the manager. You can use a custom lifecycle prefix or a completely
custom outer route as long as that callback request is forwarded to the same
named Durable Object. Deriving the callback from the current request preserves
those custom route choices.
