...
Helper-side abort is **race-free**. The per-turn `AbortController` is created at the top of `runTurnAndStream`, BEFORE the `ReadableStream` is returned to the parent. If `cancel()` fires before `saveMessages` has reached its internal `linkExternal` call, the signal is observed as already-aborted at link time and the inference is skipped entirely (zero wasted Workers AI calls). If `cancel()` fires mid-stream, the registry's controller for the request id is cancelled and the chunk loop terminates synchronously. This is the fix shipped via [`cloudflare/agents#1406`](https://github.com/cloudflare/agents/issues/1406).
...
- **Add chunk-level observability.** The helper-event protocol carries opaque `body` strings; `<HelperPanel>` decodes them into `UIMessage.parts` for rendering. A debug overlay that surfaces the raw `text-delta` / tool-call boundaries against the assistant's own message timeline is a small addition with high diagnostic value.
