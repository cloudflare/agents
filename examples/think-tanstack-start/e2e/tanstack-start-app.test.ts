...
async function waitForReady(maxAttempts = 60, delayMs = 1000): Promise<void> {
  ...
  throw new Error("TanStack Start Think app did not start in time");
}
...
describe("TanStack Start host app with Think", () => {
  ...
  it("serves TanStack Start document routes through the host app", async () => {
    const response = await fetch(`${BASE_URL}/host`);

    expect(response.status).toBe(200);
    const body = await response.text();
    expect(body).toContain("TanStack Start host:");
