...
describe("VoiceAgent — text messages", () => {
  ...
  it("streams text around two tool-call-sized pauses", async () => {
    const { ws } = await connectWS(uniquePath());
    await waitForStatus(ws, "idle");
    ...
    sendJSON(ws, { type: "text_message", text: "stream around tool calls" });

    const firstDelta = (await waitForMessageMatching(
      ws,
      (m) =>
        typeof m === "object" &&
        m !== null &&
        (m as Record<string, unknown>).type === "transcript_delta" &&
        (m as Record<string, unknown>).text === "Some intro text. ",
      100
    )) as Record<string, unknown>;
    ...
    const firstAudio = await waitForMessageMatching(
      ws,
      (m) => m instanceof ArrayBuffer,
      100
    );
    expect(firstAudio).toBeInstanceOf(ArrayBuffer);

    const middleDelta = (await waitForMessageMatching(
      ws,
      (m) =>
        typeof m === "object" &&
        m !== null &&
    ...
    expect(middleDelta).toEqual({
      type: "transcript_delta",
      text: "Here is what the first tool found. "
    });

    const secondAudio = await waitForMessageMatching(
      ws,
      (m) => m instanceof ArrayBuffer,
      100
    );
    expect(secondAudio).toBeInstanceOf(ArrayBuffer);

    const finalDelta = (await waitForMessageMatching(
      ws,
      (m) =>
        typeof m === "object" &&
        m !== null &&
        (m as Record<string, unknown>).type === "transcript_delta" &&
        (m as Record<string, unknown>).text === "Here is the conclusion.",
      350
    )) as Record<string, unknown>;

    expect(finalDelta).toEqual({
      type: "transcript_delta",
    ...
    expect(transcriptEnd.text).toBe(
      "Some intro text. Here is what the first tool found. Here is the conclusion."
    );
