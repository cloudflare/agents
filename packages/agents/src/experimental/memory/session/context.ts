...
export class ContextBlocks {
  ...
  async load(): Promise<void> {
    for (const config of this.configs) {
      ...
      const writable = config.provider
        ? isWritableProvider(config.provider) || (skill && !!(config.provider as SkillProvider).set)
        : false;
  ...
  private captureSnapshot(): string {
    ...
    for (const block of this.blocks.values()) {
      ...
      if (block.isSkill) header += " (use load_context to load)";
      if (block.maxTokens) {
