...
describe("agents:skills vite plugin", () => {
  ...
  it("bundles skill scripts as inert resources without transforming them", async () => {
    const skillsDir = join(dir, "skills");
    await mkdir(skillsDir, { recursive: true });
    ...
    expect(manifest).toContain('"kind":"script"');
    expect(manifest).toContain('from \\"./helper\\"');
    expect(manifest).toContain("type Input");
  });
