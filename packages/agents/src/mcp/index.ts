...
> extends Agent<Env, State, Props> {
  ...
  /**
   * Elicit input from the connected MCP client.
   *
   * @deprecated Prefer `this.server.server.elicitInput(...)`. This method is
   * retained for backwards compatibility and delegates to the underlying SDK
  ...
  async elicitInput(
    ...
    const innerServer = "server" in server ? server.server : server;
    return innerServer.elicitInput(params, options);
  }
