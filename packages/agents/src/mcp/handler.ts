...
export function createMcpHandler(
  serverOrFactory: McpServer | Server | (() => McpServer | Server),
  options: CreateMcpHandlerOptions = {}
): (
  request: Request,
  env: unknown,
  ...
  return async (
    ...
    const handleRequest = async () => {
      ...
      // Stateless mode: fresh transport per request. Prefer passing a factory
      // so the server instance is also fresh per request; passing an instance
      // is kept as backwards-compatible API and may fail if reused concurrently.
      const server =
