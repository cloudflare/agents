...
import type { FileInfo, FileStat } from "../experimental/workspace";
...
describe("workspace — symlinks", () => {
  ...
  it("symlink target is resolved through readlink", async () => {
    const agent = await freshAgent("sym-resolve");
    await agent.write("/src.txt", "sym content");
    await agent.symlinkCall("/src.txt", "/blink.txt");
    const content = (await agent.read("/blink.txt")) as unknown as string;
    expect(content).toBe("sym content");
  });
