...
describe("OAuth2 MCP Client - Agent compatibility", () => {
  it("constructs a restored custom provider in its Lifecycle host context", async () => {
    ...
    expect(await agentStub.getRestoreProviderAgentName()).toBe(agentName);
  });
});
...
describe("OAuth2 MCP Client - Callback Handling", () => {
  it("runs a shared callback handler in its Lifecycle request context", async () => {
    const agentName = `oauth-context-${nanoid(8)}`;
    const agentId = env.TestOAuthAgent.idFromName(agentName);
    const agentStub = env.TestOAuthAgent.get(agentId);
    const serverId = nanoid(8);
    const callbackUrl = `http://example.com/agents/test-o-auth-agent/${agentId.toString()}/callback`;
