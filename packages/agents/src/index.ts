...
export type FiberState = {
  ...
  /** Number of times the fiber has been retried (eviction or error) */
  retryCount: number;
  /** Maximum number of retries before marking as failed */
  maxRetries: number;
  /** Return value of the method on completion */
  result: unknown | null;
  /** Error message on failure */
  error: string | null;
  /** When the fiber started executing (epoch ms) */
  startedAt: number | null;
  /** When the fiber was last updated (epoch ms) */
  updatedAt: number | null;
  /** When the fiber completed (epoch ms) */
  completedAt: number | null;
  /** When the fiber was created (epoch ms) */
  createdAt: number;
};
...
export const DEFAULT_AGENT_STATIC_OPTIONS = {
  ...
  /**
   * Enable debug logging for fiber lifecycle events.
...
> extends Server<Env, Props> {
  ...
  /**
   * Set of fiber IDs actively executing in this DO instance (in-memory).
  ...
  constructor(ctx: AgentContext, env: Env) {
    ...
    // Fiber tracking table for durable long-running execution
    this.sql`
  ...
  // ── Fiber methods (durable long-running execution) ──────────────────

  /**
   * Internal heartbeat callback invoked by the scheduler.
   * Checks for fibers that were interrupted by DO eviction and triggers recovery.
   * Private — not callable via RPC and not auto-wrapped.
   * @internal
   */
  // Note: TypeScript `private` is compile-time only. The scheduler dispatches
  // callbacks by string name (`this[row.callback]`), which works at runtime.
  ...
  /**
   * Keep the Durable Object alive via a scheduled heartbeat.
   * Creates an interval schedule that persists in SQLite and survives eviction.
   * Returns a promise that resolves to a disposer function.
   *
  ...
  /**
   * Remove orphaned heartbeat schedules left behind by previous
   * DO instances that were evicted. Each recovery creates new
  ...
  private _cleanupOrphanedHeartbeats() {
    ...
    //   the recovery process (which creates new heartbeats anyway).
    // - Active fibers' disposeKeepAlive() closures will try to cancel
    //   already-deleted schedules — cancelSchedule returns false, harmless.
    // - A more precise approach would track which schedule belongs to which
    //   fiber, but the added complexity isn't worth it since this only runs
    //   during recovery (post-eviction), and recovery immediately creates
    //   fresh heartbeat schedules for all restarted fibers.
    this.sql`
  ...
  /**
   * Execute a fiber method with full lifecycle management.
  ...
  /**
   * Check for fibers that were interrupted by DO eviction.
   * Called from the alarm handler and the heartbeat callback.
    ...
    try {
      ...
      for (const fiber of runningFibers) {
        ...
        if (newRetryCount > fiber.max_retries) {
          ...
          // Mark as interrupted and prepare for recovery
          this.sql`
            UPDATE cf_agents_fibers
