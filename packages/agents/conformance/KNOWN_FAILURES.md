...
| Authorization-server migration       | `auth/authorization-server-migration`                          | A resource that changes authorization server during an existing connection may fail to recover automatically and require a reconnect. A fix needs explicit issuer-transition invalidation and re-registration semantics. |
...
| Handler Host/Origin validation       | `dns-rebinding-protection`                                     | Agents does not add localhost-only middleware. Deployments that accept arbitrary Host/Origin values must validate them at the edge or in application middleware.                                                         |
