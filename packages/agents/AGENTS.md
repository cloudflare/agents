...
- **StateMachine is a direct Lifecycle capability** — `agents/state-machine` stores complete versioned checkpoints in its own tables and drives them with scoped Lifecycle jobs. Events, waits, gates, effect recovery, cancellation, and local children commit with the checkpoint; Tasks remains an independent sibling capability.
