 ...
 * scenario against the live `*.workers.dev` URL, and ALWAYS deletes the Worker
 * in teardown. Requires an authenticated `wrangler` (run `wrangler whoami`).
 */
...
function agentWsUrl(baseUrl: string): string {
  ...
  return `${wsBase}/agents/chat-hanging-recovery-agent/${AGENT_NAME}`;
}
