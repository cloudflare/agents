...
class FailingWriteFs {
  ...
  async mkdir(path: string, options?: unknown) {
    return this.inner.mkdir(path, options as never);
  }
  ...
  async rm(path: string, options?: unknown) {
    return this.inner.rm(path, options as never);
  }

  async cp(src: string, dest: string, options?: unknown) {
    return this.inner.cp(src, dest, options as never);
  }
