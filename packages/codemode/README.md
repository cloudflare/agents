...
export class GithubConnector extends McpConnector<Env> {
  ...
  setConnection(conn: McpConnectionLike) { this.#conn = conn; }
  ...
  protected annotations() {
    return {
      ...
      create_issue: { requiresApproval: true, approvalDescription: "Create issue" },
    };
  }
}
```
...
```ts
// stripe.codemode.ts
import { OpenApiConnector, type OpenApiRequestOptions } from "@cloudflare/codemode";

export class StripeConnector extends OpenApiConnector<Env> {
  name() { return "stripe"; }
  protected instructions() { return "Use for Stripe payment operations."; }
  protected spec() { return stripeOpenApiSpec; }
...
export class LinearConnector extends ToolsetConnector<Env> {
  name() { return "linear"; }
  protected instructions() { return "Use for Linear issue tracking."; }
  protected tools() { return linearTools; }
}
```
...
Skills are reusable code patterns that combine connector methods. Any system (Think, bundled, R2) implements `CodemodeSkillSource`; codemode consumes the interface:

```ts
import type { CodemodeSkillSource } from "@cloudflare/codemode";

const skills: CodemodeSkillSource = {
  id: "my-skills",
  async list() {
    return [
      {
        name: "repo-overview",
        description: "Get a combined overview of a repository.",
...
createProxyTool({ executor, connectors, skills: [skills] });
```

Skills appear in `codemode.search` results (with `kind: "skill"`) and are executable via `codemode.run("skill-name", input)`.
