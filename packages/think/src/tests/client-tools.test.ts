...
describe("Think — regeneration", () => {
  ...
  it("applies configured history windowing to the selected regeneration branch", async () => {
    const room = crypto.randomUUID();
    const agent = await freshAgent(room);
    const { ws } = await connectWS(room);
    await collectMessages(ws, 3);
    ...
    const user1 = makeUserMessage("first");
    let donePromise = waitForDone(ws);
    sendChatRequest(ws, [user1]);
    await donePromise;

    const assistant1 = ((await agent.getMessages()) as UIMessage[])[1];
    const user2 = makeUserMessage("second");
    donePromise = waitForDone(ws);
    sendChatRequest(ws, [user1, assistant1, user2]);
    await donePromise;

    const assistant2 = ((await agent.getMessages()) as UIMessage[])[3];
    const user3 = makeUserMessage("third");
    donePromise = waitForDone(ws);
    sendChatRequest(ws, [user1, assistant1, user2, assistant2, user3]);
    await donePromise;

    await agent.setHydrationByteBudgetForTest(1);
    donePromise = waitForDone(ws);
    sendChatRequest(ws, [user1, assistant1, user2, assistant2, user3], {
      trigger: "regenerate-message"
    });
    ...
    expect((await agent.getTextOnlyPromptRoles()).at(-1)).toEqual([
      "system",
      "assistant",
      "user",
      "assistant",
    ...
    await agent.setHydrationByteBudgetForTest(0);
    donePromise = waitForDone(ws);
    sendChatRequest(ws, [user1, assistant1, user2, assistant2, user3], {
      trigger: "regenerate-message"
    });
