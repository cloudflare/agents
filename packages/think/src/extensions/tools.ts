...
import { sanitizeName } from "./manager";
 ...
 * @example
 * ```ts
 * import { createExtensionTools, ExtensionManager } from "@cloudflare/think/extensions";
 *
  ...
  return {
    load_extension: tool({
      description:
        ...
        "The execute function receives (args, host) where host may provide " +
        "file, context, message, and session capabilities according to the " +
        "extension permissions and host bridge configuration. " +
