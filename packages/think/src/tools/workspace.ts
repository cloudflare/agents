...
/** @deprecated Use `ThinkWorkspace`. */
export type WorkspaceLike = ThinkWorkspace;
...
function workspaceReadOps(workspace: ThinkWorkspace): ReadOperations {
  return {
...
function workspaceWriteOps(workspace: ThinkWorkspace): WriteOperations {
  return {
    writeFile: (path, content) => writeWorkspaceFile(workspace, path, content),
    mkdir: (path, opts) =>
      workspace.fs.mkdir(normalizeWorkspacePath(path), opts)
  };
}

function workspaceEditOps(workspace: ThinkWorkspace): EditOperations {
  return {
    readFile: (path) => readTextOrNull(workspace, path),
    writeFile: (path, content) => writeWorkspaceFile(workspace, path, content)
  };
...
function workspaceListOps(workspace: ThinkWorkspace): ListOperations {
  return {
    async readDir(dir, opts) {
      const entries = await workspace.fs.readdir(normalizeWorkspacePath(dir), {
        limit:
...
function workspaceFindOps(workspace: ThinkWorkspace): FindOperations {
  return {
    async glob(pattern) {
      const { directory, relativePattern } = splitGlobPattern(pattern);
      const entries = await workspace.fs.find(directory, relativePattern);
      return Promise.all(
        entries.map((entry) =>
...
function workspaceDeleteOps(workspace: ThinkWorkspace): DeleteOperations {
  return {
    rm: (path, opts) => workspace.fs.rm(normalizeWorkspacePath(path), opts)
  };
}

function workspaceGrepOps(workspace: ThinkWorkspace): GrepOperations {
...
async function readBytesOrNull(
  ...
  try {
    const stream = await workspace.fs.readFile(normalizeWorkspacePath(path));
