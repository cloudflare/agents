...
export interface CreateExecuteToolOptions {
  ...
  /**
   * Workspace exposed through the existing codemode `state.*` interface.
...
export function createExecuteRuntime(
  ...
  if (options.state) {
    ...
    connectors.push(
      ...createWorkspaceStateConnectors(options.workspace, options.ctx)
    );
  }
  ...
  if (connectors.length === 0) {
    throw new Error(
      "createExecuteTool has nothing to expose — provide at least one of " +
        "`workspace`, `tools`, `state`, `browser`, or `connectors`."
    );
  }
