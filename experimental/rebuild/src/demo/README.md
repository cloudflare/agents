...
The demo *is the code*: each file is one module, small enough to read on a
...
sqlite). `models/ai-sdk.ts` typechecks against `vendor/node_modules/ai` but
has no local runtime — hand `aiSdkModel(...)` a real provider model in a
deployed worker (its bundler resolves `ai` normally). There is deliberately
no fallback: broken until a real provider is wired, by design.
