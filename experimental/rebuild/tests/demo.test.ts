...
import { asBranchId, asEntryId, asTurnId } from "../src/ids.js";
import { sqliteDb } from "./helpers.js";

import { therapist, debateClub } from "../src/demo/presets.js";
import { rollingWindow } from "../src/demo/context/window.js";
import { compactor } from "../src/demo/context/compactor.js";
import { librarian } from "../src/demo/context/librarian.js";
import { priorityLanes } from "../src/demo/admission/priority.js";
import { demoTools, evaluate } from "../src/demo/tools.js";
import { tollbooth } from "../src/demo/middleware/tollbooth.js";
import { planner } from "../src/demo/loops/planner.js";

const RETRY = {
  maxAttempts: 2,
  backoff: { initialMs: 5, factor: 2, maxMs: 10 }
};
...
test("planner loop: durable plan, tool step, notes, final answer", async () => {
  ...
  const latestPlan = plans[0].payload as unknown as {
    steps: string[];
    done: number;
  };
  ...
  assert.equal(
    (await view.query({ kinds: ["effect/claimed"], limit: 5 })).length,
    0
  ); // readonly → no claim
  await agent.stop();
});
...
test("debate club preset: three arguments on the log, then one answer", async () => {
  ...
  assert.deepEqual(channel.delivered, [
    "Briefly, with care: yes, please rebuild it."
  ]);
  const args = await agent.engine
    .view()
    .query({ kinds: ["debater/argument"], limit: 10 });
  assert.equal(args.length, 3);
  const personas = [...args]
    .reverse()
    .map((e) => (e.payload as unknown as { persona: string }).persona);
...
test("bouncer: no magic word, no turn — the entry stays un-acted-upon", async () => {
  ...
  await agent.waitUntilQuiescent();
  assert.equal(
    (await agent.engine.view().query({ kinds: ["turn/marker"], limit: 3 }))
      .length,
    0
  );
  assert.deepEqual(channel.delivered, []);
...
test("tollbooth: priced dice park on a 402 until payment (approval) lands", async () => {
  ...
  const requests = await agent.engine
    .view()
    .query({ kinds: ["tools/approval-requested"], limit: 3 });
  assert.equal(requests.length, 1);
  const descriptor = (
    requests[0].payload as unknown as { descriptor: { title: string } }
  ).descriptor;
  ...
  assert.equal(
    (await agent.engine.view().query({ kinds: ["effect/settled"], limit: 5 }))
      .length,
    1
  );
  await agent.stop();
...
test("compactor: aged history becomes a private summary entry the engine never understands", async () => {
  ...
  const summarizer = new MockLanguageModel(() =>
    mockText("They exchanged twenty pleasantries.")
  );
  ...
  const summaries = await agent.engine
    .view()
    .query({ kinds: ["compactor/summary"], limit: 3 });
  assert.equal(summaries.length, 1);
  assert.match(
    (summaries[0].payload as unknown as { summary: string }).summary,
    /twenty pleasantries/
  );
  ...
  assert.ok(
    ...
    `saw ${request.messages.length} messages`
  );
  await agent.stop();
});
...
test("librarian: a second model curates which entries the first one sees", async () => {
  ...
  const curator = new MockLanguageModel(() =>
    mockText("the relevant entries are #2 and #4")
  );
  const assembler = librarian({
    system: "curated",
    librarian: curator,
    shortlist: 10,
    pick: 3
  ...
  const texts = request.messages.map(
  ...
  assert.match(
    textOfFirstUserPart(curator.requests[0].messages as never),
    /#1 \[user\]/
  );
});
...
test("priorityLanes: source-aware decisions, as pure function calls", () => {
  ...
  assert.equal(
    policy.decide({ entry: msg("user", "pager"), active, queued: [] }).action,
    "preempt"
  );
  assert.equal(
    policy.decide({ entry: msg("user", "web"), active, queued: [] }).action,
    "queue"
  );
  assert.equal(
    policy.decide({ entry: msg("user", "web"), queued: [] }).action,
    "start"
  );
  assert.equal(
    policy.decide({
...
function textOfFirstUserPart(
  messages: readonly { parts: readonly { type: string; text?: string }[] }[]
): string {
