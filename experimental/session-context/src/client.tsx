...
function ToolCard({ part }: { part: ToolPart }) {
  ...
  const isSearch = part.toolName === "search_context";
  const label = isSearch
    ? part.input?.query
      ? `"${part.input.query}"`
      : ""
    : [part.input?.action, part.input?.label].filter(Boolean).join(" ");
  ...
  return (
    <Surface className="rounded-xl ring ring-kumo-line overflow-hidden">
      ...
      >
        ...
        />
        {isSearch ? (
          <MagnifyingGlassIcon size={14} className="text-blue-500" />
        ) : null}
        <Text size="xs" bold>
          {part.toolName}
        </Text>
      ...
      {open && (
        <div className="px-3 pb-3 border-t border-kumo-line space-y-2 pt-2">
          ...
          {part.output != null && (
            <pre
              className={`font-mono text-xs rounded p-2 overflow-x-auto whitespace-pre-wrap ${
                isSearch
                  ? "text-blue-600 dark:text-blue-400 bg-blue-500/5 border border-blue-500/20"
                  : "text-green-600 dark:text-green-400 bg-green-500/5 border border-green-500/20"
              }`}
            >
...
function ConnectionIndicator({ status }: { status: ConnectionStatus }) {
  ...
  return (
    <div className="flex items-center gap-2" role="status">
      ...
      <span className={`text-xs ${text}`}>{statusLabel}</span>
    </div>
  );
}
        ...
        <div className="max-w-3xl mx-auto px-5 py-6 space-y-5">
          {messages.length === 0 && !isLoading && (
            <Empty
              ...
              description="This agent has access to a searchable knowledge base via AI Search. Ask questions and the agent will search for relevant information. It also saves facts to memory."
            />
          )}
        ...
        >
          <div className="flex items-end gap-3 rounded-xl border border-kumo-line bg-kumo-base p-3 shadow-sm focus-within:ring-2 focus-within:ring-kumo-ring focus-within:border-transparent transition-shadow">
            <InputArea
              ...
              placeholder={
                isConnected
                  ? "Ask a question — the agent can search the knowledge base..."
                  : "Connecting..."
              }
