...
| `rfc-tasks-state-machine.md`             | RFC        | Tasks as one durable state-machine engine with two definition forms — the step API compiled onto it — plus a normative specification appendix (proposed)                        |
| `tasks-cleanup-plan.md`                  | design doc | Forward plan, not current state: surface-by-surface verdicts, consumer migrations and a sequenced PR order for the Tasks engine cutover; delete once it lands                   |
