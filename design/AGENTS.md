...
| `rfc-harness-capability.md`              | RFC        | `agents/harness` — one developer API for every harness (session/prompt/interrupt/reply/events), a `HarnessRuntime` port, and a container-backed Claude Code runtime driven over Cap'n Web (proposed) |
