...
| Scheduler Lifecycle primitive and Agent alarm arbitration                | ✅ `schedules-export.test-d.ts` | ✅ `schedule-timing.test.ts` (pure timing), `tests/lifecycle/scheduler-capability.test.ts` (capability + fake services + real storage) | ✅ `tests/lifecycle/`, `schedule.test.ts`, `alarms.test.ts`, `migration.test.ts`, `keep-alive.test.ts`, `run-fiber.test.ts`, `sub-agent.test.ts` |                                                                                             —                                                                                             |                 ⚠️ indirect through fiber/recovery e2e                 |                        —                         |            —             |
